...
public class ManagerVisual_LineRenderEndless : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        while (spawnTimes.Count > 0 && spawnTimes.Peek() + lifetime < Time.time) { RemovePoint(); }

        //move positions
        velocityAux = new Vector3(velocity.x, velocity.y, velocity.z);
        Vector3 diff = -velocityAux * Time.deltaTime;
